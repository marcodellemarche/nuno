...
- [ ] Nextcloud adapter: health with write probe, list accounts, configured quota, usage with unknown detection, `NormalizeQuota` (FR-20, FR-22, FR-23)
- [ ] Immich adapter: same read path, usage cross-checked against `/api/server/statistics`, plus a write probe (the write path is the one thing never exercised against the live stack) (FR-21, FR-22, FR-23)
...
- [ ] `nuno doctor`: every precondition checked, every failure paired with the command that fixes it (FR-76)
...
Precondition, not code: the target Nextcloud must move its default from `oidc_login_default_quota` to `files/default_quota` first. Nuno cannot do this itself (it is a system config, reachable only through `occ`, and reaching `occ` would mean root on the host: see [ADR-0023](docs/decisions.md#adr-0023-nuno-diagnoses-it-does-not-reconfigure-the-services-it-manages)), so `nuno doctor` prints the commands and FR-75 keeps the provider degraded until they are run.
