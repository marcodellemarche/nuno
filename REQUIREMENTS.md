...
| FR-47 | Usage responses MUST carry per-provider `observed_at` and `status`, MUST represent unlimited as `null`, MUST NOT emit `NaN`, MUST report the observed ceiling rather than a desired one, MUST carry a stable `user_uuid` and a stable ordering, and MUST mark a ceiling left over from a policy that no longer allocates that provider. `status` MUST distinguish `unknown` (the call succeeded, the values did not) from `unavailable`, a numeric field MUST be read only together with `status`, and each user entry MUST carry `complete`, stating whether every provider contributed. See [ADR-0026](docs/decisions.md#adr-0026-the-usage-contract-needs-a-name-for-unknown). | MVP |
...
| FR-76 | Nuno MUST provide `nuno doctor`, checking every precondition (write-capable credentials, competing writers, provider versions, identity reachability, unresolved links) and printing, for each failure, the exact command that fixes it. The same text MUST appear at startup and in the UI when a provider is degraded. `nuno doctor --fix` MUST emit those commands as a runnable script and MUST NOT execute them itself (NFR-7). `nuno doctor` also owns the write probe, which MUST target a linked account and MUST NOT run from `Health()`. See [ADR-0025](docs/decisions.md#adr-0025-health-reads-the-write-probe-is-a-separate-step) and [ADR-0027](docs/decisions.md#adr-0027-a-competing-writer-nuno-cannot-see). | MVP |
...
| FR-75 | An adapter MUST detect every competing writer the provider's API exposes, and MUST refuse to **apply** against an instance where one is present, naming the remedy. Reading and reporting continue, marked degraded: observation is safe and M1 is read-only. On Nextcloud `ldapQuotaAttribute` and `ldapQuotaDefault` are app config and MUST be detected this way. `oidc_login_default_quota` is system config and is not reachable through OCS at all: it MUST be diagnosed by the script FR-76 emits, and MUST NOT be claimed as a startup check. See [ADR-0027](docs/decisions.md#adr-0027-a-competing-writer-nuno-cannot-see). | MVP |
